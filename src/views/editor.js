...
function highlightSyntax(rawContent) {
...
async function saveFileToBackend(fileName, codeContent) {
  ...
  try {
    ...
    if (data.status === 'success') {
      console.log(`[EDITOR_AUTO_SAVE] Successfully saved ${fileName} to backend workspace.`);
    } else {
      console.error(`[EDITOR_AUTO_SAVE] Failed to save:`, data.message);
    }
  } catch (error) {
    console.error(`[EDITOR_AUTO_SAVE] Network error saving file:`, error);
  }
}
...
export async function loadActiveProjectFiles(activeId) {
  try {
    ...
    function addFile(name, rawContent) {
      ...
      const highlighted = highlightSyntax(rawContent);
    ...
    if (results.backend && Array.isArray(results.backend.components)) {
      results.backend.components.forEach(comp => {
        if (comp.name && comp.code) {
          addFile(comp.name, comp.code);
        }
      });
      ...
      if (results.backend.backend_code) {
        addFile('main.py', results.backend.backend_code);
      }
...
function renderCodeTab(fileName) {
  ...
  // Retrieve the raw code content
  const rawPath = (fileName === 'package.json' || fileName === 'README.md') ? fileName : `src/${fileName}`;
  const rawContent = projectRawFiles[rawPath] || "";

  container.className = "flex-1 overflow-hidden flex bg-[#0d0e15] text-[#a6accd] relative font-mono select-text";
  container.innerHTML = `
    <!-- Line numbers -->
    <div class="text-[#4b526d] p-4 pr-3 text-right select-none flex flex-col w-12 shrink-0 code-font text-[14px] leading-[1.7] border-r border-[#1a1c25] overflow-hidden" id="line-numbers-container" style="font-family: Consolas, 'Fira Code', monospace; line-height: 1.7; font-size: 14px;">${linesHtml}</div>
    
    <!-- Code Textarea & Highlighter overlay -->
    <div class="flex-1 relative h-full overflow-hidden">
      <textarea id="editor-textarea" class="absolute inset-0 w-full h-full p-4 bg-transparent caret-[#528bff] border-none outline-none resize-none code-font text-[14px] leading-[1.7] whitespace-pre overflow-auto font-mono z-10" style="color: transparent; -webkit-text-fill-color: transparent; font-family: Consolas, 'Fira Code', monospace; line-height: 1.7; font-size: 14px; outline: none; margin: 0;" spellcheck="false" autocomplete="off" autocapitalize="off">${escapeHtml(rawContent)}</textarea>
      <pre id="editor-highlight-overlay" class="absolute inset-0 w-full h-full p-4 pointer-events-none code-font text-[14px] leading-[1.7] whitespace-pre overflow-hidden font-mono z-0" style="font-family: Consolas, 'Fira Code', monospace; line-height: 1.7; font-size: 14px; margin: 0; border: none; padding: 1rem;">${file.content}</pre>
    </div>
  `;
  ...
  if (textarea && highlightOverlay && lineNumbers) {
    // Fill raw text value safely
    textarea.value = rawContent;

    // Keep scrolling in perfect sync
    ...
    // Handle professional tab indentation key
    textarea.addEventListener('keydown', (e) => {
      if (e.key === 'Tab') {
        ...
        // Trigger input event to update highlights and trigger debounced auto-save
    ...
    textarea.addEventListener('input', () => {
      ...
      // Update raw files dictionary
      projectRawFiles[rawPath] = updatedCode;

      // Update file model lines count
      const lineCount = updatedCode.split('\n').length;
      file.linesCount = lineCount;

      // Update line numbers display dynamically
      const newLinesHtml = Array.from({ length: lineCount }, (_, i) => `<span>${i + 1}</span>`).join('');
      lineNumbers.innerHTML = newLinesHtml;

      // Update highlight overlay content
      const highlighted = highlightSyntax(updatedCode);
      file.content = highlighted;
      highlightOverlay.innerHTML = highlighted;

      // Reset scroll sync just in case
      highlightOverlay.scrollTop = textarea.scrollTop;
      highlightOverlay.scrollLeft = textarea.scrollLeft;
      ...
      autoSaveTimeout = setTimeout(() => {
        saveFileToBackend(fileName, updatedCode);
      }, 500); // 500ms auto-save debounce
    });
  }
}
...
function createFileInWorkspace(fileName) {
  ...
  // 4. Save file to backend
  saveFileToBackend(cleanName, rawContent);

  // 5. Display elegant toast notification
  showToast(`Created file: ${cleanName}`, "success");

  // 6. Re-render full file explorer components
  renderExplorerUI();
